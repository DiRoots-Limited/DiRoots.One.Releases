...
# ViewAligner
{: .no_toc }
Easily replicate the alignment of a reference view across multiple sheets to quickly achieve consistent sheet layouts.  
## Table of contents
{: .no_toc .text-delta }
...
- A list of sheets and their views is displayed, using the same browser organization as selected in Revit.

- Filter by V/S Sets - if you have organized the project sheets into sets, you can apply filters to make the process easier.

- Use the search bar to quickly locate views and select them for alignment.

- “Similar Views Only” - Shows only views that are considered similar
  - Similar views are those with the same type and the same scale as the reference view.
...
- "Hide Unchecked" - Hides unchecked sheets so you only preview the views and sheets that will be aligned.
